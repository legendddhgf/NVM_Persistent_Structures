...
typedef struct BTreeNode {
  uintptr_t *keys; // contain BTREEKEYUPPERLIMIT
  Generic *values; // ""
...
BTNode *BTNodeCopy(BTNode *n) {
  ...
  for (uintptr_t i = 0; i < BTREECHILDUPPERLIMIT; i++) {
    btn->children[i] = n->children[i];
  }
  for (uintptr_t i = 0; i < BTREEKEYUPPERLIMIT; i++) {
    btn->keys[i] = n->keys[i];
...
uintptr_t BTNodeIsLeaf(BTNode *n) {
  if (n == NULL) {
...
uintptr_t BTNodeIsFull(BTNode *n) {
  if (n == NULL) {
...
uintptr_t BTNodeNextKeyLocation(BTNode *n, uint64_t key) {
  if (n == NULL) {
...
uintptr_t BTNodeAddLocation(BTNode *n, uint64_t key) {
  if (n == NULL) {
...
uintptr_t BTNodeMedianIndex(BTNode *n, uint64_t key) {
...
BTNode *BTNodeHighestSplit(BTNode *root, uintptr_t key) {
...
BTNode *BTNodeSplitHelper(BTNode **pchild1, uintptr_t insertKey,
    Generic insertValue) {
  BTNode *child2 = NULL;
  ...
  uintptr_t thisKey = 0; // will be overwritten in following if-else
  ...
  uintptr_t child2InsertLoc = BTNodeNextKeyLocation(child1, insertKey) + 1;

  // get median index to push to parent
  uintptr_t medianIndex = BTNodeMedianIndex(child1, thisKey);
  ...
  uintptr_t parentKey = 0; // information to be sent to parent via split node
  ...
  if (medianIndex == BTREEKEYLOWERLIMIT + 1) { // key is median
    ...
    for (uintptr_t i = medianIndex; i < BTREEKEYUPPERLIMIT - 1; i++) {
    ...
    uintptr_t keyInsertLoc = BTNodeAddLocation(child1, thisKey);
    for (int64_t i = BTREEKEYUPPERLIMIT - 2; i >= (int64_t) keyInsertLoc; i--) {
  ...
  if (child2InsertLoc < BTREECHILDLOWERLIMIT) { // in child1
    ...
    for (int64_t i = BTREECHILDLOWERLIMIT - 2; i >= 0; i--) {
    ...
    for (int64_t i = BTREECHILDLOWERLIMIT - 2; i >= (int64_t) child2InsertLoc;
        i--) {
...
void BTNodeSplit(BTree *bt, uintptr_t key, Generic value) {
  BTNode *root = (BTNode *) *bt;
  ...
  uintptr_t parent_index = 0;

  uintptr_t child_index = 0; // need to save this for later recursive call
  ...
  uintptr_t keyInsertLoc = BTNodeAddLocation(highestSplitParentCopy,
...
void BTreeInsert(BTree *bt, Generic data, int64_t index) {
  ...
  BTNode *root = *bt;
  uintptr_t key = GenericHash(data);
  ...
  BTNode *iter = root;
  BTNode *parent = NULL;
  uintptr_t childIndex = 0; // index of iter relative to parent
  ...
  if (!BTNodeIsFull(iter)) {
    uintptr_t keyIndex = BTNodeAddLocation(iter, key); // get insert location
    newIter = BTNodeCopy(iter);

    // shift things right once from keyIndex to BTREEKEYUPPERLIMIT - 1
    for (int64_t i = BTREEKEYUPPERLIMIT - 2; i >= (int64_t) keyIndex; i--) {
...
Generic BTreeGetElement(BTree *bt, int64_t key) {
  ...
  for (iter = 0; iter < BTREEKEYUPPERLIMIT; iter++) {
    if (root->values[iter] == NULL || key < (int64_t) root->keys[iter]) {
      break;
    }
    if ((int64_t) root->keys[iter] == key) {
...
void BTreePrintHelper(FILE* out, BTNode *root, uintptr_t currentDepth) {
  if (root == NULL) {
    return;
  }
  for (int i = 0; i < BTREEKEYUPPERLIMIT; i++) {
